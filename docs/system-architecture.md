...
│  ┌─────────────┐ ┌──────────────┐ ┌──────────┐  │
│  │ Google      │ │ OpenAI       │ │  OCR     │  │
│  │ Translate   │ │ GPT-4o-mini  │ │ Service  │  │
│  │ (free,      │ │ (SSE stream, │ │ (Vision) │  │
│  │  default)   │ │  context)    │ │          │  │
│  └─────────────┘ └──────────────┘ └──────────┘  │
│                                                   │
│  ┌─────────────────────────────────────┐         │
...
| **GoogleTranslateProvider** | `Services/GoogleTranslateProvider.swift` | Free Google Translate via unofficial endpoint (default) |
| **OpenAITranslationProvider** | `Services/OpenAITranslationProvider.swift` | GPT-4o-mini API calls (premium, context-aware) |
...
| **UpdateService** | `Services/UpdateService.swift` | GitHub Releases version check, ZIP download, app replacement, relaunch |
...
User types text in popover
  ...
  → TranslationService.translate(text, perMessageContext)
    ...
    → activeProvider.translate(...)
      → Google Translate: GET translate.googleapis.com (free, no key)
      → OpenAI: POST api.openai.com/v1/chat/completions (key required)
...
```
All 3 merged into system prompt sent to GPT-4o-mini. Google Translate ignores context (plain text only).
